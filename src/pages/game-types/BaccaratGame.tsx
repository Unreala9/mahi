      ...
      <div className="min-h-screen bg-gradient-to-br from-green-900 via-emerald-900 to-teal-900">
        <div className="bg-slate-800 border-b border-slate-700 px-4 py-3">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-slate-400 text-sm">Round: {gameData?.mid || "---"}</p>
            </div>
            <div className="text-right">
              <p className="text-2xl font-bold text-yellow-400">{gameData?.lt || 0}s</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_400px]">
          <div className="p-6">
            ...
            <div className="grid grid-cols-2 gap-6 mb-6">
              <Card className="bg-gradient-to-br from-blue-900 to-blue-950 border-blue-700 p-6">
                <h3 className="text-white text-xl font-bold mb-4 text-center">PLAYER</h3>
                <div className="flex justify-center gap-2">
                  {playerCards.length > 0 ? playerCards.map((c, i) => (
                    <div key={i} className="w-16 h-24 bg-white rounded-lg shadow-xl flex items-center justify-center text-2xl">{c}</div>
                  )) : [0,1,2].map(i => (
                    <div key={i} className="w-16 h-24 bg-slate-700 rounded-lg"></div>
                  ))}
                </div>
              </Card>

              <Card className="bg-gradient-to-br from-red-900 to-red-950 border-red-700 p-6">
                <h3 className="text-white text-xl font-bold mb-4 text-center">BANKER</h3>
                <div className="flex justify-center gap-2">
            ...
            <div className="grid grid-cols-3 gap-4 mb-6">
              {gameData?.sub?.filter((m: any) => ["Player", "Banker", "Tie"].includes(m.nat)).map((market: any) => (
                <Button
                  key={market.sid}
                  onClick={() => handleMarketClick(market)}
                  disabled={market.gstatus === "SUSPENDED"}
                  className={`h-28 text-2xl font-bold ${
                    market.nat === "Player" ? "bg-blue-600" :
                    market.nat === "Banker" ? "bg-red-600" : "bg-green-600"
                  }`}
                >
                  <div>
                    <div>{market.nat}</div>
              ...
              <div className="flex gap-2">
                {resultData?.res?.slice(0, 10).map((r: any, i: number) => (
                  <div key={i} className={`w-10 h-10 rounded-full flex items-center justify-center font-bold ${
                    r.win === "P" ? "bg-blue-600" : r.win === "B" ? "bg-red-600" : "bg-green-600"
                  } text-white`}>{r.win}</div>
                ))}
              </div>
            </div>
          </div>
          ...
          <div className="bg-slate-800 border-l border-slate-700 p-4">
              ...
              <div className="grid grid-cols-3 gap-2">
                {chips.map((chip) => (
                  <Button key={chip} onClick={() => setSelectedChip(chip)}
                    variant={selectedChip === chip ? "default" : "outline"} className="h-12">
                    ₹{chip}
                  </Button>
                ))}
              </div>
                  ...
                  <Card key={idx} className="p-3 bg-slate-700">
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <Button size="sm" variant="ghost"
                        ...
                        className="h-6 w-6 p-0 text-red-400">×</Button>
                    </div>
            ...
            <Button onClick={handlePlaceBets} disabled={bets.length === 0}
