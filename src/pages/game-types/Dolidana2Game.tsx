...
interface Dolidana2GameProps {
  game?: CasinoGame;
}

export default function Dolidana2Game({ game }: Dolidana2GameProps) {
  const [bets, setBets] = useState<any[]>([]);
  const [selectedChip, setSelectedChip] = useState<number>(100);

  const gameId = game?.gmid || "dolidana2";
  const gameName = game?.gname || "Dolidana 2";
  ...
  const colors = [
    { name: "Red", color: "from-red-500 to-red-700", odds: 2.0 },
    { name: "Green", color: "from-green-500 to-green-700", odds: 2.0 },
    { name: "Yellow", color: "from-yellow-500 to-yellow-700", odds: 2.0 },
    { name: "Purple", color: "from-purple-500 to-purple-700", odds: 2.0 },
  ];
            ...
            <div className="lg:col-span-2 bg-black/30 backdrop-blur-sm rounded-xl p-6 shadow-2xl">
              ...
              <div className="grid grid-cols-2 gap-4">
                {colors.map((color) => (
                  <button
                    key={color.name}
                    onClick={() => handlePlaceBet(color.name, color.odds)}
                    className={`bg-gradient-to-br ${color.color} hover:opacity-90 rounded-lg p-12 transition-all transform hover:scale-105`}
                  >
                    <div className="text-3xl font-bold">{color.name}</div>
                    <div className="text-5xl font-extrabold mt-2">
                      {color.odds.toFixed(2)}
                    </div>
                  </button>
                ))}
              </div>
