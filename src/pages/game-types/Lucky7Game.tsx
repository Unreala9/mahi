        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_400px]">
          <div className="p-6">
            ...
            <div className="text-center mb-6">
              <h3 className="text-yellow-400 font-bold text-2xl mb-4">DRAWN CARD</h3>
              {drawnCard ? (
            ...
            <div className="grid grid-cols-3 gap-4 mb-6">
              {gameData?.sub?.filter((m: any) => ["Low", "7", "High"].includes(m.nat)).map((market: any) => (
                <Button
                  key={market.sid}
                  onClick={() => handleMarketClick(market)}
                  disabled={market.gstatus === "SUSPENDED"}
                  className={`h-32 text-3xl font-bold ${
                    market.nat === "Low" ? "bg-blue-600" :
                    market.nat === "7" ? "bg-green-600" : "bg-red-600"
                  }`}
                >
                  <div>
                    <div>{market.nat}</div>
              ...
              <div className="flex gap-2">
                {resultData?.res?.slice(0, 10).map((r: any, i: number) => (
                  <div key={i} className={`w-10 h-10 rounded-full flex items-center justify-center font-bold ${
                    r.win === "1" ? "bg-blue-600" : r.win === "2" ? "bg-green-600" : "bg-red-600"
                  } text-white`}>{r.win === "1" ? "L" : r.win === "2" ? "7" : "H"}</div>
                ))}
              </div>
            </div>
          </div>
