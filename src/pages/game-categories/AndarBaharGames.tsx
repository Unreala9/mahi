/**
 * Andar Bahar Games Category Page
 * Handles: ab20, abj, ab3, ab4
 */
...
import { ArrowLeft, TrendingUp, Trophy, Info, Timer } from "lucide-react";
...
export default function AndarBaharGames() {
  ...
  const [isLoading, setIsLoading] = useState(false); // Instant render
  ...
  const renderCards = (cardString: string) => {
    ...
    const andarCards = cards.slice(1).filter((_, i) => i % 2 === 0);
    const baharCards = cards.slice(1).filter((_, i) => i % 2 === 1);

    return (
      <div className="space-y-6">
        {/* Joker Card */}
        <div className="text-center">
          <Badge className="mb-2">Joker</Badge>
          <div className="w-20 h-28 bg-white rounded-lg border-4 border-yellow-500 flex items-center justify-center text-3xl font-bold shadow-lg mx-auto">
            {jokerCard || "?"}
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-6">
          <div className="text-center">
            <Badge className="mb-2 bg-green-600">Andar</Badge>
            <div className="flex flex-wrap gap-2 justify-center min-h-[100px]">
              {andarCards.length > 0 ? (
                andarCards.map((card, index) => (
                  <div
                    key={index}
                    className="w-14 h-20 bg-white rounded border-2 border-green-500 flex items-center justify-center text-xl font-bold shadow-md"
                  >
                    {card}
                  </div>
                ))
          ...
          <div className="text-center">
            <Badge className="mb-2 bg-orange-600">Bahar</Badge>
            <div className="flex flex-wrap gap-2 justify-center min-h-[100px]">
              {baharCards.length > 0 ? (
                baharCards.map((card, index) => (
                  <div
                    key={index}
                    className="w-14 h-20 bg-white rounded border-2 border-orange-500 flex items-center justify-center text-xl font-bold shadow-md"
                  >
                    {card}
                  </div>
                ))
    ...
    <div className="container mx-auto px-4 py-6 max-w-7xl">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          {gameData.card && (
            <Card className="p-8 bg-gradient-to-br from-amber-800 to-orange-900">
              <h3 className="text-white text-center font-semibold mb-6 text-xl">
                Cards
              </h3>
          ...
          <div className="space-y-3">
            <h3 className="font-semibold text-lg">Betting Markets</h3>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    {market.l && (
                      <Button
                        ...
                        onClick={() => handlePlaceBet(market, "lay", market.l!)}
                        disabled={market.gstatus === "SUSPENDED"}
                      >
                        <span className="text-xs text-muted-foreground">
                          LAY
